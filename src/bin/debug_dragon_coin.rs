...
fn check_sprite(rom_bytes: &[u8], id: u8) {
    ...
    let tiles = smwe_emu::emu::sprite_oam_tiles(&mut cpu, id);
    if tiles.is_empty() {
    ...
    for (i, t) in tiles.iter().enumerate() {
        let pal_raw = (t.tile_word >> 9) & 7;
        let tile_idx = (t.tile_word & 0x1FF) as usize + 0x600;
        println!(
            "  0x{:02X}[{}]: tile_word={:04X} tile_idx=0x{:03X} pal_raw={} cgram_row={} 16x16={} dx={} dy={}",
            id,
            i,
            t.tile_word,
            tile_idx,
    ...
    // Show palette row colors for the first tile
    let pal_raw = (tiles[0].tile_word >> 9) & 7;
...
fn main() {
    ...
    for slot in 0..64usize {
        ...
        if y < 0xE0 {
            ...
            println!(
                "  slot {:02}: y={:3} tile={:04X} tile_idx=0x{:03X} pal={} 16x16={} vram_nz={}",
                slot,
                y,
                tile,
                tile_idx,
    ...
    for slot in 0..64usize {
        ...
        if y < 0xE0 {
            println!(
                "  slot {:02}: y={:3} tile={:04X} pal={} 16x16={}",
                slot,
                y,
                tile,
                (tile >> 9) & 7,
