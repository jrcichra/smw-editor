...
/// address of its 4 tile words by running LM's own resolver routine at
/// $06F540 in the emulator (exact for any LM version); falls back to LM's
/// default extended-Map16 location when the resolver is absent.
fn lm_map16_ptr(cpu: &mut Cpu, block_id: u16) -> u32 {
    smwe_emu::emu::lm_ext_map16_data_addr(cpu, block_id)
        .unwrap_or_else(|| 0x0F8000 + ((block_id as u32 >> 8) - 2) * 0x800 + (block_id as u32 & 0xFF) * 8)
}
