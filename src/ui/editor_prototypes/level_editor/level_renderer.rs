...
impl LevelRenderer {
    ...
    fn load_layer(&mut self, gl: &Context, cpu: &mut Cpu, rom: &smwe_rom::SmwRom, fg_bg_gfx: u8, bg: bool) {
        ...
        // Lunar Magic ROMs relocate the BG Map16 table (and can edit the copy);
        // resolve the current level's base by running LM's own resolver, with
        // the vanilla table as fallback.
        let map16_bg = smwe_emu::emu::lm_bg_map16_base(cpu)
...
/// address of its 4 tile words by running LM's own resolver at $06F540 in the
/// emulator (exact for any LM version). Returns 0 when no resolver exists so
/// the caller can fall back to the static ROM parse.
fn get_lm_map16_ptr(cpu: &mut Cpu, block_id: u16) -> u32 {
    smwe_emu::emu::lm_ext_map16_data_addr(cpu, block_id).unwrap_or(0)
}
