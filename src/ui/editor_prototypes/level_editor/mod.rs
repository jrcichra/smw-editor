...
impl UiLevelEditor {
    pub fn new(gl: Arc<glow::Context>, rom: Arc<SmwRom>) -> Self {
        ...
        let mut editor = Self {
            ...
            rom,
            level_renderer,
            level_num: 0x105, // Default to a more representative vanilla level
        ...
        log::info!("Created level editor with level_num = {:#X}", editor.level_num);

        // Try to load the level explicitly
        editor.load_level();

        // Check if the level loaded successfully
        let level_idx = editor.level_num as usize;
        if level_idx < editor.rom.levels.len() {
            log::info!(
                "Successfully loaded level {:#X} with {} tiles",
                editor.level_num,
                editor.rom.levels[level_idx].layer1.as_bytes().len()
            );
        } else {
            log::error!("Failed to load level {:#X} - index out of range", editor.level_num);
        }
        editor
    }
}
...
impl UiLevelEditor {
    pub(super) fn load_level(&mut self) {
        ...
        let ext_count = self.layer1.objects.iter().filter(|o| o.is_extended).count();
        log::info!(
            "Level {:#X}: length={}, screens={}, vertical={}, objects={}, exits={}, extended_objects={}",
            self.level_num,
            self.level_properties.level_length,
            self.level_properties.num_screens(),
            self.level_properties.is_vertical,
            self.layer1.objects.len(),
            self.layer1.exits.len(),
            ext_count
        );
    ...
    fn upload_gfx_palette(&self) {
        ...
        for row in 0..16_usize {
            for col in 0..16_usize {
                let color =
                    palette.get_color_at(row, col).unwrap_or(smwe_rom::graphics::palette::ColorPalettes::TRANSPARENT);
